...
class data_base_schema(BaseModel):
    ...
    problem:str = Field(...,description="what problem occured in the user's vehicle in detail")
...
class data_fetch_slots(BaseModel):
    ...
    key_value:str = Field(...,description="if the key is garage_name then the key_value would be A motors or B motors etc it would the name of the mechanic garage, if key is slot then key_value would be 2025-11-10 or 2025-11-25")

class data_log_slots(BaseModel):
    customer_name:str = Field(...,description="the name of the customer")
    garage_name:str = Field(...,description="name of the garage customer chose")

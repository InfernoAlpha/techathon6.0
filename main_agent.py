...
from agent_node_tools import llm_agent_init,Data_Agent_init,Diagnosis_Agent_init,parser_node_init,comunication_init,tts_conv_init,condition_def,feedback_init
...
tools = [fetch_data,log_currnet_data,model_inference]
data_tool_Node = ToolNode([fetch_data])
diagnosis_tool_Node = ToolNode([model_inference])
...
graph.add_node("Diagnosis_node",llm_agent_init([model_inference]))
...
graph.add_node("RCA_CAPA",llm_agent_init([fetch_data]))
...
graph.add_conditional_edges("tts",condition_def,{"true":"feedback_init","false":END})

graph.add_edge("feedback_init","feedback_tts")
graph.add_edge("feedback_init","RCA_CAPA")
graph.add_edge("RCA_CAPA",END)
agent = graph.compile()
